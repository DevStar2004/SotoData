...
<html lang="en">
  ...
  <body>
    ...
    <div class="container mt-5 mb-5">
      ...
      $q = $pdo->prepare('SELECT COUNT(`title`) as count FROM `jobs`');
      $q->execute();
      $jobs = $q->fetch(PDO::FETCH_ASSOC)['count'];

      $q = $pdo->prepare('SELECT COUNT(`names`) as count FROM `people`');
      $q->execute();
      $people = $q->fetch(PDO::FETCH_ASSOC)['count'];

      echo '<h4 style="border-bottom: 1px solid #ccc; padding-bottom: 5px;">'.number_format($firmCount).' firms active</h4>';
      echo '<h4 style="border-bottom: 1px solid #ccc; padding-bottom: 5px;">'.number_format($jobs).' jobs</h4>';
      echo '<h4 style="border-bottom: 1px solid #ccc; padding-bottom: 5px;">'.number_format($people).' attorneys</h4>';
      ?>
