<?php

include '../config.php';
include '../simple_html_dom.php';

$base_url = 'https://www.dorsey.com';
$spider_name = 'dorsey';

$q = $pdo->prepare('DELETE FROM `queue` WHERE `spider_name`=?');
$q->execute(array($spider_name));


$values = array();

foreach (range('A', 'Z') as $key => $char) {

    $data = file_get_contents('http://137.184.158.149:3000/?api=get2&element=viewMore&url='.urlencode($base_url.'/people#search/-/'.$char.'/all/all/all/all/all/sort/p1'));
    $html = str_get_html($data);

    if($html)
    {
        foreach($html->find('.person-result') as $item)
        {

            if(!empty($item->find('img', 0)->src))
            {
                $image = @$item->find('img', 0)->src;
            }
            else
            {
                $image = '';
            }

            $email = '';
            foreach($item->find('a') as $link)
            {
                if(strpos($link->href, 'mailto:') !== false || strpos($link->href, '&#109;&#97;&#105;') !== false)
                {
                    $email = str_replace('mailto:', '', html_entity_decode($link->href));
                }
            }

            $phone = '';
            foreach($item->find('a') as $link)
            {
                if(strpos($link->href, 'tel:') !== false)
                {
                    $phone = str_replace('tel:', '', $link->href);
                }
            }

            if($item->find('a.name', 0))
            {

                $name = trim(preg_replace('/\s+/', ' ', $item->find('a.name', 0)->plaintext));

                $row = array(
                    'url' => $base_url.$item->find('a', 0)->href,
                    'name' => $name,
                    'image' => $image,
                    'title' => @trim($item->find('[class="role"]', 0)->plaintext),
                    'location' => @trim($item->find('.office a', 0)->plaintext),
                    'phone' => @trim($item->find('.office span', 0)->plaintext),
                    'email' => @trim(get_string_between($item->innertext, 'javascript:openEmailDisclaimer(\'', '\'')),
                );
                $values[] = $row;

                $q = $pdo->prepare('INSERT INTO `queue` VALUES (?, ?, ?, ?, ?, ?)');
                $q->execute(array(
                    $spider_name,
                    $row['url'],
                    json_encode($row),
                    'pending',
                    time(),
                    NULL
                ));

            }

        }
    }

}

echo count($values);

?><br/>