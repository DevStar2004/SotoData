...
foreach ($q as $row) {
    ...
    $values['description'] = $html->find('.bio__text.body__content', 0)->innertext;
    ...
    $education = array();
    $education_html = @str_get_html(get_string_between($html, 'Admissions', '</div>'));
    if(!empty($education_html))
    {
        foreach($education_html->find('p') as $item)
        {
            if(strpos($item->plaintext, 'Admitted') !== false)
            {
    ...
    $languages = array();
    $languages_html = @str_get_html(get_string_between(strtolower($html), 'languages', '</div>'));
    ...
    $practice_html = @str_get_html(get_string_between($html, '<span class="paragraph text--weight-bold margin-bottom--20 block">Practices</span>', '</ul>'));
