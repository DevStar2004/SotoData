...
$base_url = 'https://www.shumaker.com';
$spider_name = 'shumaker';
$firm_name = 'Shumaker, Loop & Kendrick, LLP';
...
foreach ($q as $row) {
    ...
    $pData['location'] = $html->find('.sidebar.sidebar-right .card a', 2)->plaintext;
    $pData['name'] = $html->find('.page-head-title', 0)->plaintext;
    $pData['phone'] = $html->find('.sidebar.sidebar-right .card a', 0)->plaintext;
    $pData['email'] = html_entity_decode($html->find('.sidebar.sidebar-right .card a', 1)->plaintext);
    $pData['title'] = $html->find('.page-head-subtitle', 0)->plaintext;
    $pData['image'] = html_entity_decode($html->find('.bio img', 0)->src);
    ...
    $pData['name'] = explode(', ', $html->find('.page-head-title', 0)->plaintext)[0];
    ...
    foreach($html->find('.accordion-item-holder') as $item)
    {
        if(strpos(strtolower($item->innertext), 'service lines') !== false)
        {
