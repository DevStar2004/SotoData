...
$firm_name = 'Kilpatrick Townsend & Stockton LLP';
...
foreach ($q as $row) {
    ...
    $pData['location'] = trim(explode(',', $html->find('.address', 0)->plaintext)[1]);
    ...
    $positions = json_encode(array($html->find('.bio-profile', 0)->plaintext));

    $values['description'] = superTrim($html->find('.abstractBold.peopleabstrachBold', 0)->plaintext);
    ...
    if(!empty($education_html))
    {
        ...
        {
            if(strpos(strtolower($item->plaintext), 'university') !== false)
            {
                $education[] = superTrim(str_replace(array('Of', 'And'), array('of', 'and'), ucwords(trim($item->plaintext))));
            }
    ...
    foreach($html->find('.col-sm-6') as $item)
    {
