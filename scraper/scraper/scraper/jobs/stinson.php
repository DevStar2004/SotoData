...
foreach ($q as $row) {
    ...
    $row['url'] = str_replace('https://www.stinson.compeople', 'https://www.stinson.com/people', $row['url']);
    $pData['url'] = $row['url'];
    ...
    $pData['location'] = trim(strip_tags(get_string_between($html, '<span>LOCATION:</span>', '</li>')));
    ...
    $practice_area_html = @str_get_html(get_string_between($html, '<h2>Practices &amp; Industries</h2>', '</ul>'), 2);
