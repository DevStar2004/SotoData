...
$spider_name = 'bipc';
$firm_name = 'BUCHANAN INGERSOLL & ROONEY PC';
...
foreach ($q as $row) {
    ...
    $values['description'] = $html->find('[data-briefcase="bioContent"] .wrap', 0)->innertext;
 
    $photo = $pData['image'];
    $thumb = $pData['image'];

    $education = array();
    $education_html = @str_get_html(get_string_between($html, '<div class="header">Education</div>', '</div>'));
    ...
    $admission_html = @str_get_html(get_string_between($html, 'Admissions</div>', '</ul>'));
    ...
    $practice_areas = array();
    foreach($html->find('.widget.link-nav') as $item)
    {
        if(strpos(strtolower($item->innertext), 'related services') !== false)
        {
            foreach($item->find('a') as $practice)
            {
                $practice_areas[] = trim(ucwords(strtolower($practice->plaintext)));
            }
        }
    }
    ...
    $primaryAddress = $html->find('.phone-locations__location', 0)->plaintext;
