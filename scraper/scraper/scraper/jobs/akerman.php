...
foreach ($q as $row) {
    ...
    $c = $pdo->prepare('SELECT * FROM `people` WHERE `source`=? LIMIT 1');
    $c->execute(array($row['url']));
    if($c->rowcount()>0 || empty($data) || !$html->find('body', 0))
    {
    ...
    if(isset($values['vCard']))
    {
        ...
        $q->execute(array(json_encode($values['names'])));
