...
$spider_name = 'curtis';
$firm_name = 'Curtis, Mallet-Prevost, Colt & Mosle LLP';
...
foreach ($q as $row) {
    ...
    $data = fetch_quick($row['url'].'/credentials');
    $credentials = str_get_html($data);
    ...
    $values['description'] = $html->find('.pg-layout__sidebar-block-txt', 0)->innertext;
 
    $photo = $pData['image'];
    $thumb = $pData['image'];

    $education = array();
    $education_html = @str_get_html(get_string_between(strtolower($credentials), 'education', '</div>'));
    ...
    $languages = array();
    $languages_html = @str_get_html(get_string_between($html, 'Languages', '</div>'));
    ...
    $admission_html = @str_get_html(get_string_between($credentials, 'Admitted to Practice', '</div>'));
    ...
    foreach($html->find('.pg-layout__sidebar-block.pg-layout__sidebar-block--border') as $item)
    {
        if(strpos(strtolower($item->innertext), 'focus areas') !== false)
        {
    ...
    foreach($education as $value)
    {
        ...
        if(strpos($school, 'jd') !== false || strpos($school, 'jurisprudence') !== false || strpos($school, 'doctor') !== false)
        {
            $law_school = $value;
            break;
        }
