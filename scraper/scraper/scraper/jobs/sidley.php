...
foreach ($q as $row) {
    ...
    $education = $pData['Schools'];
    $pData['email'] = $pData['BusinessEmail'];
    $pData['name'] = @$html->find('#main-content', 0)->plaintext;
    $pData['phone'] = $pData['BusinessPhone'];
    $pData['title'] = $pData['ExecLevel'];
    $pData['image'] = $pData['PhotoUrl'];
    $pData['location'] = $pData['Offices'][0]['Name'];
    ...
    $positions = @json_encode(array($pData['AttorneyLevelTitle']));

    $values['description'] = superTrim($html->find('.rich-text-content', 0)->innertext);;
    ...
    $practice_area_html = @str_get_html(get_string_between($html, '<div class="category-name">SERVICES AND INDUSTRIES</div>', '</ul>'), 2);
