...
foreach ($q as $row) {
    ...
    else
    {
        ...
        $primaryAddress = trim($html->find('.office-contact a', 0)->plaintext);
        ...
        $phone_numbers[] = $pData['office_1_phone'];

        foreach($html->find('.expando__body.js-expando__body') as $ul)
        {
            if(strpos(
        ...
        foreach($pData['content_data']['bar_admissions'] as $item)
        {
            $bar_admissions[] = $item['name'];
        }
        ...
        $findUL = str_get_html(get_string_between($html->innertext, 'Practices</h3>', '</ul>'));
        if(!empty($findUL))
        {
            foreach($findUL->find('li') as $li)
            {
        ...
        $description = $pData['main_content'];

        foreach($html->find('img') as $item)
        {
            if(strpos(strtolower($item->src), '/images/content') !== false)
            {
                $image = $item->src;
                break;
            }
