...
$firm_name = 'Ogletree, Deakins, Nash, Smoak & Stewart, P.C.';
...
foreach ($q as $row) {
    ...
    $values['description'] = superTrim($html->find('.entry-content.wysiwyg', 0)->innertext);;
 
    $photo = $pData['image'];
    $thumb = $photo;

    $education = array();
    $education_html = @str_get_html(substr(get_string_between($html, '<h3 class="field-label h5">Education</h3>', '</ul>'), 2));
    ...
    $admission_html = @str_get_html(get_string_between($html, '<h3 class="field-label h5">Admittance to Practice</h3>', '</ul>'));
    ...
    if($ul = $html->find('.content-areas-of-practice', 0))
    {
