...
$spider_name = 'ebglaw';
$firm_name = 'Epstein Becker & Green, P.C.';
...
foreach ($q as $row) {
    ...
    $data = fetch_quick($base_url.'/'.$row['url']);
    ...
    $values['description'] = $html->find('.wp-block-contentpilot-introduction', 0)->innertext;
    ...
    if($ul = $html->find('.widget.bar-admissions', 0))
    {
    ...
    if($ul = $html->find('.widget.court-admissions', 0))
    {
        foreach($ul->find('li') as $item)
        {
    ...
    $practice_html = @str_get_html(get_string_between($html, 'Services</h3>', '</ul>'));
