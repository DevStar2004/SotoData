...
foreach ($q as $row) {
    ...
    foreach($html->find('a') as $link)
    {
        if(strpos($link->href, 'email-protection') !== false || strpos($link->href, '&#109;&#97;&#105;') !== false)
        {
            $pData['email'] = $values['email'] = cfDecodeEmail(str_replace('/cdn-cgi/l/email-protection#', '', $link->href));;
            break;
        }
    }
    ...
    $positions = json_encode(array($html->find('.bioInfoWrap div', 0)->plaintext));

    $values['description'] = $html->find('.intro-text.wb-content', 0)->innertext;
 
    $photo = $pData['image'];
    $thumb = $pData['image'];

    $education = array();
    $education_html = @str_get_html(get_string_between($html, '<h3 class="subHead">Education</h3>', '</ul>'));
    if(!empty($education_html))
    {
        foreach($education_html->find('li') as $item)
        {
            $education[] = str_replace(array('Of', 'And'), array('of', 'and'), ucwords(trim($item->innertext)));
        }
    }
    ...
    $admission_html = @str_get_html(get_string_between($html, '<h3 class="subHead">Admissions</h3>', '</ul>'));
    ...
    $practice_html = @str_get_html(get_string_between($html, '<h3 class="subHead">Services</h3>', '<div class="edu-wrap">'));
