...
$spider_name = 'mto';
$firm_name = 'Munger, Tolles &amp; Olson LLP';
...
foreach ($q as $row) {
    ...
    $pData['location'] = $html->find('.bio-contacts li', 0)->plaintext;
    $pData['name'] = $html->find('.bio-title', 0)->plaintext;
    $pData['phone'] = $html->find('.bio-contacts a', 0)->plaintext;
    $pData['email'] = html_entity_decode($html->find('.bio-contacts a', 1)->plaintext);
    $pData['title'] = '';
    $pData['image'] = $html->find('img.print-show', 0)->src;
    ...
    $values['description'] = $html->find('.readmoreless.assetWrapper.TextAsset', 0)->innertext;
 
    $photo = $base_url.$pData['image'];
    $thumb = $photo;

    $education = array();
    $education_html = @str_get_html(substr(get_string_between($html, '>Education</div>', '</ul>'), 2));
    ...
    $practice_area_html = @str_get_html(get_string_between($html, 'Practice Areas</div>', '</ul>'), 2);
