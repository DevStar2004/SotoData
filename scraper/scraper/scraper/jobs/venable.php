...
foreach ($q as $row) {
    ...
    $data = @fetch('http://137.184.158.149:3000/?api=get&useProxy=1&url='.urlencode($row['url']));
    $pData = @json_decode($row['data'], 1);

    if(empty($pData['FullName']))
    {
        continue;
    }
    else
    {
        ...
        $names = explode(' ', trim($pData['FullName']));
        ...
        if(!empty($pData['Offices'][0]['Label']))
        {
        ...
        $findUL = str_get_html(get_string_between($html->innertext, '<h3>Bar Admissions</h3>', '</ul>'));
        if(!empty($findUL))
        {
        ...
        $findUL = str_get_html(get_string_between($html->innertext, '<h3>Court Admissions</h3>', '</ul>'));
        if(!empty($findUL))
        {
            foreach($findUL->find('li') as $li)
            {
        ...
        $findUL = str_get_html(get_string_between($html->innertext, 'Related Practices</h3>', '</ul>'));
        if(!empty($findUL))
        {
            foreach($findUL->find('li') as $li)
            {
        ...
        foreach($html->find('p') as $item)
        {
            ...
            {
                $description .= preg_replace('/\s+/', ' ', $item->innertext);
            }
        }
        ...
        {
            $image = $base_url.$pData['Image']['Url'];
        }
