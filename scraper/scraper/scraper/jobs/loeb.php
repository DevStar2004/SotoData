...
foreach ($q as $row) {
    ...
    $arr = array();
    foreach($html->find('.profile-hero__locations a') as $item)
    {
        $arr[] = $item->plaintext;
    }

    $pData['location'] = trim(implode(', ', $arr));
    ...
    $values['description'] = $html->find('.richtext', 0)->innertext;
 
    $photo = $pData['image'];
    $thumb = $photo;

    $education = array();
    $education_html = @str_get_html(substr(get_string_between($html, '<h3 class="content-combo__heading fontsize--4">Education</h3>', '</ul>'), 2));
    ...
    if($ul = $html->find('.services-panel__list', 0))
    {
