...
foreach ($q as $row) {
    ...
    foreach($html->find('a') as $link)
    {
        ...
        {
            $values['email'] = str_replace('mailto:', '', html_entity_decode($link->href));
        }
    }

    $positions = json_encode(array($pData['title']));

    $values['description'] = $html->find('.content-container p', 0)->innertext;
 
    $photo = $base_url.$pData['image'];
    $thumb = $base_url.$pData['image'];

    $education = array();
    $education_html = @str_get_html(get_string_between($html, '<h5 class="">Education</h5>', '</div>'));
    ...
    if($ul = $html->find('[data-custom-tab-page="credentials"] ul', 3))
    {
    ...
    if($ul = $html->find('[data-custom-tab-page="credentials"] ul', 4))
    {
        foreach($ul->find('li') as $item)
        {
    ...
    foreach($html->find('.columns.small-12.medium-6') as $item)
    {
