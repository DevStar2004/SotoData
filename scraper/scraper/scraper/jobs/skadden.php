...
$spider_name = 'skadden';
$firm_name = 'Skadden, Arps, Slate, Meagher & Flom LLP';
...
foreach ($q as $row) {
    ...
    $pData['phone'] = $pData['PrimaryPhone'];
    $pData['location'] = $pData['PrimaryOffice']['Name'];
    $pData['image'] = $pData['Photo'];
    $pData['email'] = $pData['Email'];
    $pData['title'] = $pData['Level'];
    ...
    $values['description'] = superTrim($html->find('.overview-wrapper', 0)->innertext);;
    ...
    foreach($html->find('.capabilities-list a') as $item)
    {
