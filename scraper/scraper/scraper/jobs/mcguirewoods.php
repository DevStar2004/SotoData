...
foreach ($q as $row) {
    ...
    $pData['location'] = superTrim($html->find('.contact-list-item', 0)->plaintext);
    $pData['phone'] = superTrim($html->find('.contact-list-item', 1)->plaintext);
    ...
    $values['description'] = $html->find('.experience-rte', 0)->innertext;
    ...
    $practice_area_html = @str_get_html(get_string_between($html, 'Practices', '</ul>'), 2);
