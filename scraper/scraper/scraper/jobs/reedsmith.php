...
foreach ($q as $row) {
    ...
    $pData['phone'] = $pData['Phone'];
    $pData['title'] = $pData['Title'];
    $pData['location'] = $pData['Offices'][0]['City'];
    $pData['image'] = $html->find('.professional-header-figure-img', 0)->src;
    ...
    $values['description'] = superTrim($html->find('.read-more-intro.rte', 0)->innertext);;
 
    $photo = $base_url.$pData['image'];
    $thumb = $photo;

    $education = array();
    $education_html = @str_get_html(substr(get_string_between($html, '<div id="education" aria-hidden="false" class="rte accordion-node-content js-accordion-content">', '</ul>'), 2));
    ...
    $practice_area_html = @str_get_html(get_string_between($html, '<header class="section-header"><h3 class="section-header-title">Capabilities</h3></header>', '</ul>'), 2);
