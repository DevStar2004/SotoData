...
$spider_name = 'wnj';
$firm_name = 'Warner Norcross and Judd LLP';
...
foreach ($q as $row) {
    ...
    $row['url'] = str_replace('https://www.wnj.comhttps://www.wnj.com', 'https://www.wnj.com', $row['url']);
    ...
    $pData['title'] = $html->find('li.position', 0)->plaintext;
    ...
    $values['description'] = superTrim($html->find('.fade.tab-pane', 0)->innertext);
    $pData['location'] = $html->find('.RelatedOffice_office__ntoFo', 0)->plaintext;
    ...
    $admission_html = @str_get_html(get_string_between($html, '<h2>admitted</h2>', '</ul>'));
    ...
    $practice_area_html = @str_get_html(get_string_between($html, '<h2>practices</h2>', '</ul>'), 2);
