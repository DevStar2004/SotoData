...
$firm_name = 'Frost Brown Todd LLP';
...
foreach ($q as $row) {
    ...
    $values['description'] = '';
    foreach($html->find('.col') as $item)
    {
        if(strpos($item->plaintext, 'Biography') !== false)
        {
            $values['description'] = str_replace('Biography', '', $item->plaintext);
        }
    }
    ...
    $education = array();
    $education_html = @str_get_html(get_string_between($html, '<h2 class="h4">Education</h2>', '<div class="acc-item">'));
    ...
    $admission_html = @str_get_html(get_string_between($html, '<h2 class="h5">Jon is licensed to practice in the following state(s):</h2>', '<div class="acc-item">'));
    ...
    foreach($html->find('.wrapper .row .col') as $item)
    {
