...
$firm_name = 'Miller, Canfield, Paddock and Stone, P.L.C.';
...
foreach ($q as $row) {
    ...
    try {
        $pData['image'] = '/'.$html->find('#bioPhoto', 0)->src;
    } catch (Exception $e) {
        $pData['image'] = '';
    }

    $pData['location'] = trim($html->find('#bioOffices a', 0)->plaintext);
    ...
    $values['description'] = $html->find('.staticList', 0)->plaintext;
 
    $photo = $base_url.$pData['image'];
    $thumb = $photo;

    $education = array();
    $education_html = @str_get_html(get_string_between($html, '<h2>Education</h2>', '</div>'));
