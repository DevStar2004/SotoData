...
foreach ($q as $row) {
    ...
    $admission_html = @str_get_html(get_string_between($html, '<h3 class="h4-size content-block-title">Admissions</h3>', '</ul>'));
    ...
    if($html->find('.detail-content-practices .pdf-only', 0))
    {
        foreach($html->find('.detail-content-practices .pdf-only', 0)->find('li') as $item)
        {
