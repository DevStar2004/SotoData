...
foreach ($q as $row) {
    ...
    $pData['phone'] = $html->find('.nrf-tel-link', 0)->plaintext;
    ...
    $values['description'] = $html->find('.container.my-5.two-columns-container .col-12.col-md-8 div', 0)->plaintext;
 
    $photo = $base_url.$pData['image'];
    $thumb = $photo;

    $education = array();
    if($ul = $html->find('#education', 0))
    {
        foreach($ul->find('div') as $item)
        {
    ...
    $practice_area_html = @str_get_html(get_string_between($html, '<h3 class="h5 nrf-related-services-title">Key industry sectors</h3>', '</ul>'), 2);
