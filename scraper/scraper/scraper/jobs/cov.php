...
$spider_name = 'cov';
$firm_name = 'Covington & Burling LLP';
...
foreach ($q as $row) {
    ...
    $values['description'] = $html->find('.col-12.col-lg-7.see-more', 0)->innertext;

    if($html->find('.hero__image img', 0))
    {
        $image = $html->find('.hero__image img', 0)->src;
    }
    else
    {
        $image = '';
    ...
    if($ul = $html->find('.col-12.col-lg-4.offset-lg-1.d-none.d-lg-block ul', 0))
    {
        foreach($ul->find('li a') as $item)
        {
    ...
    $primaryAddress = trim($html->find('.hero__content__phone-numbers__location', 0)->plaintext);
