...
foreach ($q as $row) {
    ...
    try {
        $pData['phone'] = trim($html->find('.attorney-header__info--number', 0)->plaintext);
    } catch (Exception $e) {
        $pData['phone'] = '';
    }
    ...
    $values['description'] = @$html->find('.field--type-text-with-summary', 0)->innertext;
    ...
    {
        foreach($ul->find('.field--item') as $item)
        {
    ...
    if($ul = $html->find('.field--name-field-industry', 0))
    {
        foreach($ul->find('.field--item') as $item)
        {
