...
foreach ($q as $row) {
    ...
    $values['description'] = superTrim($html->find('#bio', 0)->innertext);;
    ...
    foreach($html->find('.bkg-gray-1.p-4.p-lg-5') as $item)
    {
        if(strpos(strtolower($item->innertext), 'related services') !== false)
        {
            foreach($item->find('a') as $practice)
            {
                $practice_areas[] = $practice->plaintext;
            }
        }
    }

    unset($practice_areas[0]);
    array_splice($practice_areas, -2);
