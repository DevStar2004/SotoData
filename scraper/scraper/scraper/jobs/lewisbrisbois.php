...
$firm_name = 'Lewis Brisbois Bisgaard & Smith LLP';
...
foreach ($q as $row) {
    ...
    $pData['title'] = superTrim($html->find('.sub-heading', 0)->plaintext);
    $pData['location'] = $html->find('.location strong', 0)->plaintext;
    ...
    $values['description'] = strip_tags($html->find('#biography p', 0)->plaintext);

    if($html->find('#hero img', 0))
    {
        $image = $html->find('#hero img', 0)->src;
    }
    else
    {
        $image = '';
    ...
    if($ul = $html->find('#practiceAreas_mobile', 0))
    {
