...
foreach ($q as $row) {
    ...
    $ex = explode('<br/>', superTrim($html->find('.attorney-contact-info p', 0)->innertext));

    if(!$html->find('.attorney-photos img', 0))
    {
        $pData['image'] = '';
    }
    else
    {
        $pData['image'] = $html->find('.attorney-photos img', 0)->src;
    }
    ...
    $pData['email'] = trim(strip_tags(html_entity_decode($ex[2])));
    
    $pData['title'] = superTrim($html->find('.attorney-position', 0)->plaintext);

    if(!$html->find('.col-xs-6.col-sm-4', 1))
    {
        $pData['location'] = '';
    }
    else
    {
        $pData['location'] = trim($html->find('.col-xs-6.col-sm-4', 1)->find('a', 0)->plaintext);
    }
    ...
    $values['description'] = superTrim($html->find('.tab-content.js-read-more', 0)->innertext);;
 
    $photo = $pData['image'];
    $thumb = $photo;

    $education = array();
    $education_html = @str_get_html(substr(get_string_between($html, '<h4>Education</h4>', '</ul>'), 2));
    ...
    $admission_html = @str_get_html(get_string_between($html, '<h4>Bar Admissions</h4>', '</ul>'));
    ...
    $practice_area_html = @str_get_html(get_string_between($html, '<h4>Practice Areas</h4>', '</ul>'), 2);
