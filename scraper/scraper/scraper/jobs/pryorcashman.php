...
foreach ($q as $row) {
    ...
    $data = @fetch('http://137.184.158.149:3000/?api=get2&useProxy=1&url='.urlencode($row['url']));
    ...
    else
    {
        ...
        $names = explode(' ', trim($html->find('#page-title', 0)->plaintext));
        ...
        foreach($html->find('a') as $item)
        {
            if(strpos($item->href, '/office-locations/') !== false)
            {
        ...
        $positions[] = $html->find('h3', 0)->plaintext;
