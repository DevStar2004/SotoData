...
foreach ($q as $row) {
    ...
    if($html->find('.role', 0))
    {
        $positions = json_encode(array($html->find('.role', 0)->plaintext));
    }
    else
    {
        $positions = '[]';
    ...
    $values['description'] = $html->find('.lessCopy', 0)->innertext;
    ...
    foreach(@$html->find('.bar-admission-section .area') as $item)
    {
    ...
    foreach($html->find('.focus-area') as $item)
    {
    ...
    if($html->find('.people-info-section .region', 0))
    {
        $primaryAddress = $html->find('.people-info-section .region', 0)->plaintext;
    }
    else
    {
        $primaryAddress = '';
