...
foreach ($q as $row) {
    ...
    $pData['phone'] = $pData['offices_info'][0]['repeater_module_office']['phone'];
    $pData['title'] = $pData['content_data']['position']['name'];
    $pData['location'] = $pData['content_data']['offices_info-repeater_module_office-office'][0]['name'];
    $pData['image'] = $pData['w452_url'];
    ...
    $values['description'] = superTrim($html->find('.rte', 0)->innertext);;
    ...
    $practice_area_html = @str_get_html(get_string_between($html, '<div class="type__navigation">Areas Of Focus</div>', '<div class="grid__pageMargins'));
