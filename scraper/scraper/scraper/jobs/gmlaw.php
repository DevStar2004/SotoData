...
foreach ($q as $row) {
    ...
    $values['description'] = $html->find('#panel1', 0)->innertext;
    ...
    $admission_html = @str_get_html(get_string_between($html, 'Admissions', '</ul>'));
    ...
    $practice_html = @str_get_html(get_string_between($html, '<h3>Practice Areas</h3>', '</ul>'));
    ...
    $primaryAddress = '';
    foreach($html->find('.biography_info li') as $item)
    {
        if(strpos($item->innertext, '<i class="fa fa-location-arrow" aria-hidden="true"></i>') !== false)
        {
            $primaryAddress = trim(explode(', ', $item->plaintext)[2]);
        }
    }
