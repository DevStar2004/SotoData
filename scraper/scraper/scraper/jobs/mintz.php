...
$spider_name = 'mintz';
$firm_name = 'Mintz, Levin, Cohn, Ferris, Glovsky and Popeo, P.C.';
...
foreach ($q as $row) {
    ...
    {
        if(strpos($item->href, 'email-protection') !== false)
        {
            $pData['email'] = trim(cfDecodeEmail(str_replace('/cdn-cgi/l/email-protection#', '', $item->href)));
            break;
        }
    }
    ...
    $values['description'] = strip_tags($html->find('.wysiwyg', 0)->plaintext);
 
    $photo = $pData['image'];
    $thumb = $photo;

    $education = array();
    $education_html = @str_get_html(substr(get_string_between($html, '<h3 class="sidebar-list__title" data-trigger>Education</h3>', '</ul>'), 2));
    ...
    $practice_area_html = @str_get_html(get_string_between($html, '<h3 class="sidebar-list__title" data-trigger>Related Practices</h3>', '</ul>'), 2);
