...
$firm_name = 'Hughes Hubbard & Reed LLP';
...
foreach ($q as $row) {
    ...
    $values['phone_numbers'] = json_encode(array(trim($html->find('.headers__contact-row p', 1)->plaintext)));
    ...
    $values['description'] = $html->find('.text-section__content', 0)->innertext;
    ...
    $admission_html = @str_get_html(get_string_between($html, '<strong>Bar Admissions</strong>', '</ul>'));
    ...
    foreach($html->find('.mob-accordion') as $item)
    {
        if(strpos(strtolower($item->innertext), 'areas of focus') !== false)
        {
    ...
    $primaryAddress = trim($html->find('.headers__contact-row p', 0)->plaintext);
