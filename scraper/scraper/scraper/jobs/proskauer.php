...
foreach ($q as $row) {
    ...
    $pData['location'] = trim($html->find('.office-h3-max-width a', 0)->plaintext);
    ...
    $values['description'] = superTrim($html->find('.generic-content-container', 0)->innertext);;
    ...
    $practice_area_html = @str_get_html(get_string_between($html, '<div class="accordion-two">', '</ul>'), 2);
    if(!empty($practice_area_html))
    {
        foreach($practice_area_html->find('a') as $item)
        {
            if($item->plaintext != 'Related Practices')
            {
