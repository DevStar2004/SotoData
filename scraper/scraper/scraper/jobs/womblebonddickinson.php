...
foreach ($q as $row) {
    ...
    $data = @fetch('http://137.184.158.149:3000/?api=get0&url='.urlencode($row['url']));
    ...
    else
    {
        ...
        $primaryAddress = @trim($html->find('.field--name-field-profile-location', 0)->plaintext);
        ...
        foreach($html->find('a') as $item)
        {
            ...
            {
                $phone_numbers[] = trim(str_replace('tel:', '', $item->href));
                break;
            }
        }
        ...
        $findUL = str_get_html(get_string_between($html->innertext, 'Related Services</h2>', '</article>'));
        if(!empty($findUL))
        {
        ...
        $positions[] = trim($html->find('.field--name-field-job-title', 0)->plaintext);

        $description = @trim($html->find('.paragraph.paragraph--type--text-area.paragraph--view-mode--default', 0)->plaintext);
