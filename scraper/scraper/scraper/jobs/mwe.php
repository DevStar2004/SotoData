...
foreach ($q as $row) {
    ...
    $values['description'] = $html->find('.row.default-margin-bottom.default-text-editor', 0)->plaintext;
 
    $photo = $pData['image'];
    $thumb = $photo;

    $education = array();
    $education_html = @get_string_between($html, '<p><strong>Education</strong><br />', '</p>');
    $ex = explode('<br />', $education_html.' <br />');
    foreach($ex as $item)
    {
        if(!empty($item))
        {
    ...
    if($ul = $html->find('.related-services-content-box', 0))
    {
