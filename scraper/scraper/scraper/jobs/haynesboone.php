...
$firm_name = 'Haynes and Boone, LLP';
...
foreach ($q as $row) {
    ...
    $values['description'] = $html->find('.col-lg-12', 1)->plaintext;
 
    $photo = $base_url.$pData['image'];
    $thumb = $base_url.$pData['image'];

    $education = array();
    $education_html = @str_get_html(get_string_between($html, '<p class="people-bio-label">Education</p>', '</div>'));
    if(!empty($education_html))
    {
        foreach($education_html->find('p') as $item)
        {
            $education[] = str_replace(array('Of', 'And'), array('of', 'and'), ucwords(trim(preg_replace('/\s+/', ' ', $item->plaintext))));
        }
    }
    ...
    $admission_html = @str_get_html(get_string_between($html, '<p class="people-bio-label">Admissions</p>', '</div>'));
    ...
    $practice_html = @str_get_html(get_string_between($html, '<ul class="related-practice-list">', '</ul>'));
