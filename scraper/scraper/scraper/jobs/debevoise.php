...
$firm_name = 'Debevoise & Plimpton LLP';
...
foreach ($q as $row) {
    ...
    $values['description'] = $html->find('.PGFirstPara', 0)->innertext;
    ...
    $admission_html = @str_get_html(get_string_between($html, '<h3 class="mod__title">Bar Admissions</h3>', '</div>'));
    ...
    foreach($html->find('.mod.rel-practices') as $item)
    {
    ...
    $primaryAddress = $html->find('.contact-office__item a', 0)->plaintext;
