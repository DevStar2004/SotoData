...
foreach ($q as $row) {
    ...
    $pData['phone'] = str_replace('//', '', $pData['phone']);
    ...
    foreach($html->find('[aria-label="Practices Widget"]') as $item)
    {
    ...
    $values['description'] = $html->find('.overview-content', 0)->innertext;

    if(!empty($pData['image']))
    {
        $pData['image'] = str_replace('//', '', $pData['image']);
        $pData['image'] = str_replace('https:', 'https://', $pData['image']);
    }
    else
    {
        $pData['image'] = '';
    ...
    $primaryAddress = $html->find('.office-link', 0)->plaintext;
