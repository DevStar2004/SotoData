...
foreach ($q as $row) {
    ...
    else
    {
        ...
        $locations = array();
        foreach($pData['offices'] as $item)
        {
            $locations[] = $item['title'];
        }

        $primaryAddress = implode(', ', $locations);
        ...
        $findUL = str_get_html(get_string_between($html->innertext, 'Admissions</h3>', '<h3 color'));
        if(!empty($findUL))
        {
            foreach($findUL->find('div') as $li)
            {
        ...
        $findUL = str_get_html(get_string_between($html->innertext, 'Practice Areas</h3>', '<h3 color'));
        if(!empty($findUL))
        {
        ...
        $positions[] = $pData['relatedTitle']['title'];
        ...
        if(!empty($pData['image']['fluid']['src']))
        {
            $image = $pData['image']['fluid']['src'];
        }
