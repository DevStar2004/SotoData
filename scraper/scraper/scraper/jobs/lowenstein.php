...
$firm_name = 'Lowenstein Sandler LLP';
...
foreach ($q as $row) {
    ...
    $pData['phone'] = $html->find('#work', 0)->plaintext;

    if(!$html->find('#Attorney-bio span', 0))
    {
    ...
    $pData['email'] = $html->find('[for="people-email"] a', 0)->plaintext;
    $pData['location'] = $html->find('#Attorney-bio span', 0)->plaintext;
    ...
    $values['description'] = strip_tags(get_string_between($html, '<h2 class="screen-reader-text">See more section</h2>', '</section>'));
    ...
    $practice_area_html = @str_get_html(get_string_between($html, '<div class="practices">', '</div>'), 2);
