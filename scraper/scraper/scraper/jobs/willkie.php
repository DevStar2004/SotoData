...
$spider_name = 'willkie';
$firm_name = 'Willkie Farr & Gallagher LLP';
...
foreach ($q as $row) {
    ...
    $pData['image'] = $html->find('.header-headshot img', 0)->src;
    ...
    $education = array();
    $education_html = @str_get_html(substr(get_string_between($html, 'Education', '</div>'), 2));
    if(!empty($education_html))
    {
        foreach($education_html->find('span') as $item)
        {
    ...
    $admission_html = @str_get_html(get_string_between($html, 'Admissions', '</span>'));
    if(!empty($admission_html))
    {
        foreach($admission_html->find('span') as $item)
        {
    ...
    $practice_areas[] = superTrim($html->find('.professional-detail-desc a', 0)->plaintext);
