...
foreach ($q as $row) {
    ...
    try {
        $pData['image'] = $html->find('.profile-heading__snapshot', 0)->src;
    } catch (Exception $e) {
    ...
    $values['description'] = superTrim($html->find('.read-more__window', 0)->plaintext);
 
    $photo = $base_url.$pData['image'];
    $thumb = $photo;

    $education = array();
    foreach($html->find('.normalized-rte-list__list li') as $item)
    {
        if(strpos(strtolower($item->innertext), 'university') !== false)
        {
            $education[] = $item->innertext;
        }
    }
    ...
    $admission_html = @str_get_html(get_string_between($html, 'Admissions & Qualifications', '</ul>'));
    ...
    $practice_area_html = @str_get_html(get_string_between($html, '<h3 class="listing-services__heading">Practices</h3>', '</ul>'), 2);
