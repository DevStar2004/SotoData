...
foreach ($q as $row) {
    ...
    foreach($html->find('.card.card--bold') as $item)
    {
        if(strpos(strtolower($item->innertext), 'industries') !== false)
        {
    ...
    $primaryAddress = $html->find('.vcard_item--location span', 0)->plaintext;
