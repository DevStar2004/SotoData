...
$spider_name = 'katten';
$firm_name = 'Katten Muchin Rosenman LLP';
...
foreach ($q as $row) {
    ...
    $pData['image'] = $html->find('.bio-banner__image', 0)->src;
    ...
    $values['description'] = strip_tags(get_string_between($html, '<div class="bio-about__introduction s-wb-content">', '</div>'));
    ...
    $practice_area_html = @str_get_html(get_string_between($html, '<h3 class="experience-sidebar-titles">Practice Areas</h3>', '</ul>'), 2);
    ...
    $q->execute(array(
        ...
        json_encode(array(trim($html->find('.bio-banner__practice-focus', 0)->plaintext))),
        '[]',
        '[]',
        $positions,
