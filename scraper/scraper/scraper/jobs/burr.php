...
foreach ($q as $row) {
    ...
    $values['description'] = $html->find('#bio_content', 0)->innertext;
 
    $photo = $base_url.'/'.$pData['image'];
    $thumb = $base_url.'/'.$pData['image'];

    $education = array();
    if($ul = $html->find('#bio_education ul', 0))
    {
    ...
    $admission_html = @str_get_html(get_string_between($html, '<h2>Admitted In</h2>', '</ul>'));
    ...
    $practice_html = @str_get_html(get_string_between($html, '<h2>Capabilities</h2>', '</ul>'));
    ...
    foreach ($html->find('a') as $item) {
        if(strpos($item->href, 'contact/') !== false)
        {
