...
$firm_name = 'Morgan, Lewis & Bockius LLP';
...
foreach ($q as $row) {
    ...
    $values['names'] = json_encode(explode(' ', ucwords($pData['name'])));
    ...
    $values['description'] = $html->find('.heading-brief', 0)->plaintext;
    ...
    $admission_html = @str_get_html(get_string_between($html, '<h2>Admissions</h2>', '</ul>'));
    ...
    $practice_area_html = @str_get_html(get_string_between($html, '<h2>Services</h2>', '</ul>'), 2);
