...
foreach ($q as $row) {
    ...
    else
    {
        ...
        $findUL = str_get_html(get_string_between($html->innertext, 'Admissions</h2>', '</ul>'));
        if(!empty($findUL))
        {
        ...
        $findUL = str_get_html(get_string_between($html->innertext, 'Languages</h2>', '</ul>'));
        if(!empty($findUL))
        {
            foreach($findUL->find('li') as $li)
            {
        ...
        $findUL = $html->find('[id="Practice Areas"]', 0);
        if(!empty($findUL))
        {
            foreach($findUL->find('li') as $li)
            {
        ...
        {
            $pData['image'] = str_replace('//', 'https://', $pData['image']);
            $image = $pData['image'];
        }
